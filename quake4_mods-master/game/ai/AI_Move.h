...
public:
	...
	struct movementFlags_s {
		...
		bool		disabled			:0;		// is movement disabled?  (this includes turning) MOD CHANGE
		bool		ignoreObstacles		:0;		// don't check for obsticles in path

		bool		allowDirectional	:0;		// allows directional movement (ie. running sideways, backwards)
		bool		allowAnimMove		:0;		// allows any animation movement
		bool		allowPrevAnimMove	:0;		// allows slide move if current animmove has no motion extraction on it (smooth transitions)
		bool		allowHiddenMove		:0;		// allows character to still move around while hidden
		bool		allowPushMovables	:0;		// allows the articulated figure to push moveable objects
		bool		allowSlideToGoal	:0;		// allows the AI to excactly slide to the goal position if close enough
